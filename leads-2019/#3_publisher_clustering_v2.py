...
for i in range(2):
    if i == 0:
        ...
        df1.to_csv("./processed_data_10k/cluster_result_1.csv", index=False, index_label=False,
                   columns=["id", "length", "text", "publisher_pro",
                            "isbn_new", "year_min", "year_max", "record"])
    else:
        cluster_result = []
        for i in range(max(df['cluster_pre'])):
            row = {}
            row['id'] = i
            row['text'] = ";;".join(df.loc[df['cluster_pre'] == i, "publisher"])
            row['publisher_pro'] = ";;".join(df.loc[df['cluster_pre'] == i, "publisher_pro"])
            row['isbn'] = ";".join(df.loc[df['cluster_pre'] == i, "isbn_tag"])
            row['year'] = ";".join(df.loc[df['cluster_pre'] == i, "year"])
            row['record'] = sum(df.loc[df['cluster_pre'] == i, "record"])
            isbn_vector = row['isbn'].split(";")
            year_vector = row['year'].split(";")
            row['isbn_new'] = ";".join(set(isbn_vector))
            row['year_min'] = min(year_vector)
        ...
        df1 = pd.DataFrame(cluster_result)
        df1.to_csv("./processed_data_10k/cluster_result_2.csv", index=False, index_label=False,
                   columns=["id", "length", "text", "publisher_pro",
                            "isbn_new", "year_min", "year_max", "record"])

data.to_csv("./processed_data_10k/publisher_10k_p1_a.csv", index=False, index_label=False,
          columns = ["F001_a", "leader_18", "isbn_tag", "F008_0710",
                     "F040_e", "F260_is", "F264_is", "F26x_b_cleaned",
                     "F26x_b_cleaned_pro"])
df.to_csv("./processed_data_10k/pre_cluster.csv", index=False, index_label=False)