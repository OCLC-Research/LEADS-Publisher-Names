...
# This function removes all the ending full stop from the text strings.
def clean_text(x):
    x = re.sub("\\.$", "", x)
    return(x)

# This is a packaged function to extract information from marcxml file
# x should be the link to the marcxml file
# Output file printed to: "./processed_data_1m/result_1m.csv"
            ...
            if tag == '020':
                ...
                for s in subfields:
                    if s.get('code') == 'a':
                        ...
                        elif len(isbn) == 13 and is_isbn13(str(isbn)) == True and mask(isbn) is not None and isbn[
                                                                                                             0:3] == "978":
                            isbn_text = str(isbn)
                            isbn_list.append(isbn_text)
...
df = marcxml_parsing("./raw_data/marc_1MM.xml")
